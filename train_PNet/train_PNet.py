...
from models.Loss import ClassLoss, BBoxLoss, accuracy
...
for epoch in range(epoch_num):
    for batch_id, (img, label, bbox, _) in enumerate(train_loader()):
        class_out, bbox_out = model(img)
        cls_loss = class_loss(class_out, label)
        box_loss = bbox_loss(bbox_out, bbox, label)
        total_loss = radio_cls_loss * cls_loss + radio_bbox_loss * box_loss
        total_loss.backward()
        optimizer.step()
        optimizer.clear_grad()
        if batch_id % 100 == 0:
            acc = accuracy(class_out, label)
            print('[%s] Train epoch %d, batch %d, total_loss: %f, cls_loss: %f, box_loss: %f, '
                  'accuracy：%f' % (datetime.now(), epoch, batch_id, total_loss, cls_loss, box_loss, acc))
    scheduler.step()
    ...
    paddle.jit.save(layer=model,
                    path=os.path.join(model_path, 'PNet'),
                    input_spec=[InputSpec(shape=[None, 3, None, None], dtype='float32')])
